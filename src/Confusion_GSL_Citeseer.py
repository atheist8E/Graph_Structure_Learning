...
def set_args():
    ...
    parser.add_argument("--model",              type = str,        default = "GCN_GSL_Citeseer")
    parser.add_argument("--description",        type = str,        default = "GCN GSL on Citeseer")
    return parser.parse_args()

def GSL(args, G, A_star):
    A = to_dense_adj(G.edge_index, max_num_nodes = args.num_nodes)[0]
    A_mask = 1 - (torch.matmul(G.train_mask.float().unsqueeze(1), G.train_mask.float().unsqueeze(0)) + \
             torch.matmul((~G.train_mask).float().unsqueeze(1), (~G.train_mask).float().unsqueeze(0)))
    A_prime = A
    for k in range(args.num_iterations):
        A_dagger = (A_star - A_prime) * A_mask * 3327
        A_positive = F.gumbel_softmax(A_dagger, dim = 1, tau = 5, hard = True)
    ...
    G_prime = Data(x = G.x, edge_index = edge_index_prime, y = G.y, \
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.result_path, "Experiment_Description.txt"), "w") as f:
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr = args.learning_rate_0, weight_decay = 5e-3)
        ...
        for l in range(1, args.num_heads + 1):
            args.model_name = l
            G_prime = GSL(args, G, A_star)
            G_prime.cuda(args.gpu)
            model_prime = GCN_NC_GSL(args, writer).cuda(args.gpu)
            criterion = nn.CrossEntropyLoss()
            optimizer = torch.optim.Adam(model_prime.parameters(), lr = args.learning_rate_0, weight_decay = 2e-3)
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones = [args.milestone_0, args.milestone_1] , gamma = 0.1)
