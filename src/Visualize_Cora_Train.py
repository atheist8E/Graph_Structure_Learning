...
def set_args():
	...
	parser.add_argument("--description",        type = str,        default = "Train Node Visualize")
...
def visualize(dataset, result_path):
	...
	graph = dataset[0]
	for i, mask in enumerate(graph.train_mask):
		if mask == True:
			...
			labels = {i: bool(mask) for i, mask in enumerate(graph.train_mask[subset])}
